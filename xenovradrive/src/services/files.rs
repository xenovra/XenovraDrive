...
impl<'d> FilesService<'d> {
    ...
    pub async fn create_folder(
        ...
        if in_schema.folder_name.contains(r"/") {
            return Err(XenovraDriveError::InvalidFolderName);
        }
    ...
    pub async fn upload_to(&self, in_schema: InFileSchema, user: &AuthUser) -> XenovraDriveResult<()> {
        // 0. checking access
        check_access(
            &self.access_repo,
            user.id,
    ...
    async fn _upload(&self, file: File, file_data: Bytes, user: &AuthUser) -> XenovraDriveResult<()> {
    ...
    async fn check_storage_workers(&self, storage_id: Uuid) -> XenovraDriveResult<()> {
        if !self
            .storage_workers_repo
            .storage_has_any(storage_id)
            .await?
        {
            Err(XenovraDriveError::StorageDoesNotHaveWorkers)
        } else {
            Ok(())
        }
    }
