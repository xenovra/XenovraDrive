...
use super::errors::{XenovraDriveError, XenovraDriveResult};
...
impl Config {
    pub fn new() -> XenovraDriveResult<Self> {
    ...
    #[inline]
    fn get_env_var<T: FromStr>(env_var: &str) -> XenovraDriveResult<T> {
        env::var(env_var)
            .map_err(|_| XenovraDriveError::EnvConfigLoadingError(env_var.to_owned()))?
            .parse::<T>()
            .map_err(|_| XenovraDriveError::EnvVarParsingError(env_var.to_owned()))
    }

    #[inline]
    fn get_env_var_with_default<T: FromStr>(env_var: &str, default: T) -> XenovraDriveResult<T> {
        let result = Self::get_env_var(env_var);

        if matches!(result, Err(XenovraDriveError::EnvConfigLoadingError(_))) {
            return Ok(default);
        }
