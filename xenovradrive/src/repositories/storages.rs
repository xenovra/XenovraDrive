...
impl<'d> StoragesRepository<'d> {
    ...
    pub async fn create(&self, in_obj: InStorage) -> XenovraDriveResult<Storage> {
        let id = Uuid::new_v4();
        ...
        .map_err(|e| match e {
            sqlx::Error::Database(dbe) if dbe.is_foreign_key_violation() => {
                XenovraDriveError::UserWasRemoved
            }
    ...
    pub async fn list_by_user_id(&self, user_id: Uuid) -> XenovraDriveResult<Vec<StorageWithInfo>> {
        sqlx::query_as(
            format!(
                "
    ...
    pub async fn get_by_id(&self, id: Uuid) -> XenovraDriveResult<Storage> {
    ...
    pub async fn get_by_file_id(&self, file_id: Uuid) -> XenovraDriveResult<Storage> {
        sqlx::query_as(
    ...
    pub async fn delete_storage(&self, storage_id: Uuid) -> XenovraDriveResult<()> {
