...
impl<'d> StorageWorkersRepository<'d> {
    ...
    pub async fn create(&self, in_obj: InStorageWorker) -> XenovraDriveResult<StorageWorker> {
        let id = Uuid::new_v4();
        ...
        .map_err(|e| match e {
            ...
            sqlx::Error::Database(dbe) if dbe.is_foreign_key_violation() => {
                XenovraDriveError::DoesNotExist("Such storage does not exist".to_string())
            }
            _ => {
                tracing::error!("{e}");
    ...
    pub async fn storage_has_any(&self, storage_id: Uuid) -> XenovraDriveResult<bool> {
    ...
    pub async fn list_by_user_id(&self, user_id: Uuid) -> XenovraDriveResult<Vec<StorageWorker>> {
    ...
    pub async fn get_by_name_and_user_id(
        ...
        name: &str,
        user_id: Uuid,
    ) -> XenovraDriveResult<StorageWorker> {
        sqlx::query_as(&format!(
    ...
    pub async fn get_token(
        ...
        storage_id: Uuid,
        limit: u8,
    ) -> XenovraDriveResult<Option<StorageWorkerTokenOnly>> {
